...
export default function CaregiverSettings() {
  ...
  if (household === undefined || me === undefined || children === undefined) {
          ...
          <CardContent className="space-y-3 pt-0">
            <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                <Button size="sm" onClick={handleSaveHousehold}>Save</Button>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border">
          ...
          <CardContent className="pt-0 space-y-2">
            <div className="flex items-center gap-3 p-2 rounded-lg bg-muted/50">
              ...
              <div>
                <p className="text-sm font-medium">{me?.name ?? 'Caregiver'}</p>
                <p className="text-xs text-muted-foreground">Caregiver</p>
              </div>
            </div>
            {children?.map(c => (
              <div key={c._id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/50">
                <span className="text-xl">{c.avatar}</span>
                <div>
                  <p className="text-sm font-medium">{c.name}</p>
                  <p className="text-xs text-muted-foreground">Child, age {c.age ?? 0}</p>
                </div>
              </div>
            ))}
